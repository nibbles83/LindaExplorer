...
module.exports = {
  ...
  get_lindamasternodelist: function(cb) {
      var uri = base_url + 'masternode?command=status-all';
      request({uri: uri, json: true}, function (error, response, body) {
      return cb(body);
    });
  },
  ...
  get_version: function(cb) {
    request({uri: 'https://github.com/TheLindaProjectInc/Linda/releases.atom', json: true}, function (error, response, body) {
      parseString(body, function(err, result){
        var version = JSON.stringify(result.feed.entry[0].title);
